...
class FaceExtractor:
    ...
    def process_videos(self, input_dir, filenames, video_idxs):
        ...
        orig_size = np.array([1080, 1920])
        orig_size = np.tile(orig_size, batch.shape[0]).reshape(batch.shape[0], -1)
        orig_size = torch.from_numpy(orig_size)
        all_detections = self.facedet.predict_on_batch(batch, orig_size=orig_size)
